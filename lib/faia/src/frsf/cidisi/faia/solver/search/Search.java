...
public class Search extends Solve {
  ...
  public SearchAction solve(Object[] params) {
    ...
    // This iteration will occur while nodesToExpand have nodes and the actual node
    // is not a goal node.-
    ...
    if (goal && !getBestPath().isEmpty()) {
      ...
      // The first node of the branch has the action that must be executed by the
      // agent.-
  ...
  private boolean existsNode(NTree node, NTree parent) {
    ...
    // This is an iteration through the node's parent (and ancestors) looking for a
    // repeated node
    ...
    // At this point it's sure that the node does not exists in the branch of the
    // Search Tree.-
    return false;
  }
  ...
  private Vector<NTree> getBestPath() {
    ...
    // The branch's top is not the root node of the tree. This is because there is
    // no action
    // associated with the root node. So, the branch's top is a son of the root
    // node.-
    while (node.getParent() != null) {
      // I insert every node at the first position, therefore I get the path from
      // rootNode to lastNode
  ...
  public void showTree() {
    ...
    case Search.XML_TREE:
      ...
      /*
       * String arbol = this.toXml(); System.out.println("Arbol:");
